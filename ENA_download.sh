...
paste fname1 fname3 | awk 'OFS="\t"{print $2"_R1.fastq.gz", $1}' | xargs -P $njobs -L 1 -n 2 curl -o
## fetch read2
echo "Fetching Read 2 for paired reads with suffix: _R2.fastq.gz"
paste fname2 fname3 | awk '/_2.fastq.gz/ {print $2"_R2.fastq.gz", "\t", $1}' | xargs -P $njobs -L 1 -n 2 curl -o
